...
class HereMap extends Component {
  ...
  componentDidMount() {
    ...
    axios.get("https://nameless-retreat-67960.herokuapp.com/unternehmen").then(res => {
  ...
  render() {
    ...
    if (this.state.data) {
      ...
      markers = data.map(u =>

        {
         ...
         return (<Marker position={[parseFloat(u.langitude), parseFloat(u.longitude)]} icon={greenIcon}
          onMouseOver={(e) => {
            e.target.openPopup();
          }}
          onMouseOut={(e) => {
    ...
    return (
      ...
      <div>
        <Search filterValue={this.filterValue}></Search>
        <Map
          center={[45.11, 78.65]}
          zoom={14}
          maxZoom={16}
        ...
        >
          ...
          />
          {markers}
          <Marker icon={redIcon} position={[45.11, 78.65]}
            onMouseOver={(e) => {
              e.target.openPopup();
            }}
            onMouseOut={(e) => {
